...
def main():
    ...
    if training_args.do_train:
        ...
        #     validation_dataset = ContrastivePretrainDatasetDeepmatcher(validation_dataset, tokenizer=model_args.tokenizer, dataset=data_args.dataset_name, deduction_set=data_args.id_deduction_set)
        if training_args.load_best_model_at_end:
            test_dataset = raw_datasets["test"]
            test_dataset = ContrastivePretrainDatasetDeepmatcher(test_dataset, tokenizer=model_args.tokenizer, clean=data_args.clean, dataset=data_args.dataset_name, deduction_set=data_args.id_deduction_set, aug=data_args.augment, split=False)

    elif training_args.do_eval:
        if "validation" not in raw_datasets:
            raise ValueError("--do_eval requires a validation dataset")
        validation_dataset = raw_datasets["validation"]
        validation_dataset = ContrastivePretrainDatasetDeepmatcher(validation_dataset, tokenizer=model_args.tokenizer, clean=data_args.clean, dataset=data_args.dataset_name, deduction_set=data_args.id_deduction_set, aug=data_args.augment, split=False)
