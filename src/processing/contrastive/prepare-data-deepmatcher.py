...
def preprocess_dataset(arg_tuple):
    ...
    merged_ids = set()
    merged_ids.update(train_df['id_left'])
    merged_ids.update(train_df['id_right'])
    
    merged_ids.update(gs_df['id_left'])
    merged_ids.update(gs_df['id_right'])
    
    entity_set = left_df[left_df['id'].isin(merged_ids)]
    entity_set = entity_set.append(right_df[right_df['id'].isin(merged_ids)])
    ...
    entity_set = entity_set.append(single_entities)
...
if __name__ == '__main__':
    ...
    ('beeradvo-ratebeer', 'beeradvo', 'ratebeer'),
